...
var listChannels = [];
//array of scheduled and recorded entries
var listRecordline = [];
...
function scrollHandler(event) {
    ...
    //recordline close to timeline
    document.getElementById('recordline').style.top = nVScroll + document.getElementById('timeline').clientHeight + 'px';
                ...
                http.onreadystatechange = function() {
                    if (http.readyState == 4 && http.status == 200) {
                        ...
                        if (p != null) {
                            ...
                            //find movie on tmdb, but there is no poster
...
function readEpg(response) {
    if (response.entries.length > 0) {
        ...
        for (var i in response.entries) {
            ...
            if (listChannels.indexOf(response.entries[i].channelUuid) != -1) {
                ...
                //add when scheduled or recording in started to recordline
                if (e.dvrState != null)
                    listRecordline.push(e.dvrUuid, x, w);
    ...
    //draw new recordline
    initRecordline(false);
}

function cancel(id, dvrUuid, channel) {
    cancelRecordingId = id;
    //drop canceled recording from recordline
    for (i = 0; i < listRecordline.length; i = i + 3) {
        if (listRecordline[i] == dvrUuid) {
            listRecordline.splice(i, 3);
        }
    }
...
function initRecordline(init) {
    listRecordline;
    var html;
    if (init == true) {
        //initial and empty recordline
        var tl = document.getElementById('timeline');
        html = '<div style="left:0px;width:' + tl.scrollWidth + 'px;"></div>';
        append('<div id="recordline">' + html + '</div>');
    } else {
        //create recordline with scheduled and running entries
        html = '';
        for (i = 0; i < listRecordline.length; i = i + 3) {
            //if (i == 0) {
            html += '<div id="' + listRecordline[i] + '" style="left:' + listRecordline[i + 1] + 'px;width:' + listRecordline[i + 2] + 'px;"></div>';
        }
        var r = document.getElementById('recordline');
        r.innerHTML = html;
    }
}
